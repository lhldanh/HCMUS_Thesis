...
Cấu hình tuyển chọn \textbf{21 quan hệ ứng viên} giàu dữ liệu thuộc nhiều lĩnh vực (mở rộng từ 5
quan hệ lõi của CronQuestions), tổ chức thành các nhóm chủ đề như liệt kê trong
Bảng~\ref{tab:relations}. Trong số đó, \textbf{8 quan hệ} đã được xây dựng bộ mẫu câu
(\textit{template}) và thực sự tham gia sinh câu hỏi ở mô-đun \texttt{generate.py}; mỗi quan hệ
được gắn một động từ và một danh từ tiếng Việt để ghép vào mẫu câu.

\begin{table}[ht]
\centering
\caption{Các nhóm quan hệ ứng viên trong CronQ-VN. Quan hệ \textbf{in đậm} (8) đã có bộ template
và được dùng để sinh câu hỏi; phần còn lại là quan hệ ứng viên trong cấu hình.}
\label{tab:relations}
\begin{tabular}{L{3.2cm} L{9.5cm}}
\toprule
\textbf{Nhóm} & \textbf{Quan hệ (mã Wikidata)} \\
\midrule
Lõi CronQ & \textbf{chức vụ (P39)}, \textbf{câu lạc bộ thể thao (P54)}, \textbf{giải thưởng (P166)}, \textbf{hôn nhân (P26)}, \textbf{nơi làm việc (P108)} \\
Học vấn/Sự nghiệp & \textbf{học tại (P69)}, học vị (P512), nơi công tác (P937) \\
Chính trị & đứng đầu chính phủ (P6), \textbf{đảng phái (P102)}, chủ tịch (P488), giữ vị trí (P1308) \\
Giải thưởng & đề cử (P1411), thắng cuộc thi (P1346) \\
Thành viên & \textbf{thành viên của (P463)}, nơi cư trú (P551) \\
...
Câu hỏi được sinh theo \textbf{năm loại}, tương ứng năm dạng suy luận thời gian, minh họa trong
Bảng~\ref{tab:qtype-examples}. Mô-đun \texttt{generate.py} \emph{liệt kê toàn bộ} câu hỏi hợp lệ mà
bộ mẫu câu và đồ thị có thể tạo ra (không lấy mẫu), nên phân bố giữa các loại phản ánh mật độ dữ
kiện thực tế của từng loại trên đồ thị (Bảng~\ref{tab:q-stats}):
