...
Quan hệ & 271 \\
Quan hệ ứng viên trong cấu hình & 21 \\
Quan hệ có template (thực sự sinh câu hỏi) & 8 \\
\bottomrule
\end{tabular}
\end{table}

Từ đồ thị này, bộ câu hỏi CronQ-VN được sinh bằng cách \emph{liệt kê toàn bộ} câu hỏi hợp lệ (không
lấy mẫu), thu được \textbf{539.584} câu. Bảng~\ref{tab:q-stats} trình bày phân bố theo loại: do sinh
vét cạn nên phân bố phản ánh mật độ dữ kiện thực tế của từng loại (không đồng đều). Đáp án dạng thực
thể chiếm đa số ($\sim$63\%) so với đáp án thời gian ($\sim$37\%, chỉ từ \texttt{simple\_time}) --
một đặc điểm cần lưu ý khi phân tích kết quả theo kiểu đáp án. Để so sánh công bằng giữa các loại,
pha đánh giá dùng \textbf{lấy mẫu phân tầng} cân bằng theo loại (Mục~\ref{sec:setup}).
...
\midrule
\texttt{simple\_entity} & Thực thể & 99.121 \\
\texttt{simple\_time}   & Thời gian & 200.924 \\
\texttt{before\_after}  & Thực thể & 170.561 \\
\texttt{first\_last}    & Thực thể & 24.700 \\
\texttt{time\_join}     & Thực thể & 44.278 \\
\midrule
\textbf{Tổng} & & \textbf{539.584} \\
\bottomrule
\end{tabular}
\end{table}
